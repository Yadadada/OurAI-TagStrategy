import { ACADEMY_DIST, ACADEMY_HOBBY_BIAS, ACADEMY_PERSONALITY_BIAS, CAMPUS_DIST, GENDER_DIST, GRADE_DIST, HOBBIES, LONG_DISTANCE_DIST, PERSONALITY_LIKERT_BIAS, PERSONALITY_TAGS, RELATIONSHIP_GOAL_DIST, RELATIONSHIP_NEEDS, RELATIONSHIP_ROLE_DIST, ROUTINE_DIST, SMOKING_DIST, } from './distributions.js';
...
function generateDesiredGenders(rng, self) {
    // 80% heterosexual, 13% bi, 5% other-only, 2% none.
    if (!self || self === 'non_binary')
        return pickN(rng, GENDERS, randInt(rng, 1, 2));
    const r = rng();
    if (r < 0.8) {
...
function generateHobbies(rng, academy) {
...
function generatePersonalityTags(rng, academy) {
...
function generateAnswers(rng, personality, goal) {
    const baseLikert = () => clipInt(gauss(rng, 4.5, 1.4), 1, 7);
    const answers = {};
    for (let i = 1; i <= 18; i++) {
    ...
    return {
        ...
        relationship_role: pickWeighted(rng, RELATIONSHIP_ROLE_DIST),
...
function generateUsername(rng, gender, index) {
...
export function generateUsers(count, seed) {
    const rng = makeRng(seed);
    const out = [];
    const baseDate = new Date('2026-04-01T00:00:00Z').getTime();
    for (let i = 0; i < count; i++) {
        const academy = pickWeighted(rng, ACADEMY_DIST);
        const campus = pickWeighted(rng, CAMPUS_DIST);
        const grade = pickWeighted(rng, GRADE_DIST);
        const gender = pickWeighted(rng, GENDER_DIST);
        const desired = generateDesiredGenders(rng, gender);
        const smoking = pickWeighted(rng, SMOKING_DIST);
        const long_distance = pickWeighted(rng, LONG_DISTANCE_DIST);
        const routine = pickWeighted(rng, ROUTINE_DIST);
        const hobbies = generateHobbies(rng, academy);
        const personality = generatePersonalityTags(rng, academy);
        const goal = pickWeighted(rng, RELATIONSHIP_GOAL_DIST);
        const answers = generateAnswers(rng, personality, goal);
        const intro = chance(rng, 0.4) ? pick(rng, INTRO_PROMPTS) : undefined;
        if (intro)
            answers.intro_prompt = intro;
        if (chance(rng, 0.3))
            answers.q19 = pick(rng, Q19_DEALBREAKERS);
        if (chance(rng, 0.2))
            answers.q20 = pick(rng, Q20_NOTES);
        const created_at = new Date(baseDate + randInt(rng, 0, 30 * 24 * 3600 * 1000)).toISOString();
        const last_active_at = new Date(Math.min(Date.now(), new Date(created_at).getTime() + randInt(rng, 0, 7 * 24 * 3600 * 1000))).toISOString();
        out.push({
            id: pseudoId('user', i),
            ...
            profile: {
                ...
                long_distance_preference: long_distance,
                routine: routine,
                hobbies,
