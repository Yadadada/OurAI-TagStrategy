...
// 0 = E/S/T/J, 100 = I/N/F/P (matches the upstream user_portrait.mbti_ei layout).
function mbtiFromUser(user: SyntheticUser, rng: () => number): {
  ei: number; sn: number; tf: number; jp: number; type: string;
} {
  ...
  ei += (4 - a.q09) * 4;
  if (tags.has('introverted')) ei += 18;
  if (tags.has('extroverted')) ei -= 18;
  if (tags.has('homebody')) ei += 8;
  if (tags.has('talkative')) ei -= 10;
  ei += gauss(rng, 0, 6);

  // S/N: openness (q06) + creative/curious tags
  let sn = 50;
  sn += (a.q06 - 4) * 5;
  if (tags.has('creative')) sn += 14;
  if (tags.has('curious')) sn += 8;
  if (tags.has('rational')) sn -= 6;
  if (tags.has('organized')) sn -= 8;
  sn += gauss(rng, 0, 6);

  // T/F: empathetic vs rational; q05 (calm under pressure) leans T
  let tf = 50;
  if (tags.has('empathetic')) tf += 18;
  if (tags.has('sensitive')) tf += 12;
  if (tags.has('romantic')) tf += 8;
  if (tags.has('rational')) tf -= 18;
  ...
  let jp = 50;
  jp += (4 - a.q03) * 6;
  if (tags.has('organized')) jp -= 16;
  if (tags.has('perfectionist')) jp -= 10;
  if (tags.has('spontaneous')) jp += 16;
  if (tags.has('carefree')) jp += 8;
  jp += gauss(rng, 0, 6);
...
const ARCHETYPES: Record<string, { name: string; one_liner: string }> = {
...
function buildTraits(user: SyntheticUser, rng: () => number) {
  const a = user.answers;
  const tags = new Set(user.profile.personality_tags);

  const v = (mean: number, sd = 8) => clipInt(gauss(rng, mean, sd), 0, 100);
...
const HOBBY_TO_INTERESTS: Record<string, { tag: string; category: string }[]> = {
...
function buildInterests(user: SyntheticUser, rng: () => number) {
  const interests = [];
...
export function generatePortraits(users: SyntheticUser[], seed: number): SyntheticPortrait[] {
  ...
  return users.map((user) => {
    ...
    const confidence: 'low' | 'medium' | 'high' =
      sample_count >= 22 ? 'high' : sample_count >= 18 ? 'medium' : 'low';
