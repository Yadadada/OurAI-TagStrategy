...
// Deliberately different from the production baseline:
...
function isEligible(a, b) {
    if (a.id === b.id)
        return false;
    const aGender = (a.profile.gender_identity || 'non_binary');
    const bGender = (b.profile.gender_identity || 'non_binary');
    if (a.profile.desired_genders.length === 0 || b.profile.desired_genders.length === 0)
        return false;
    if (!a.profile.desired_genders.includes(bGender))
        return false;
    if (!b.profile.desired_genders.includes(aGender))
        return false;
    return true;
}
function groundTruthScore(a, b, pa, pb, rng) {
    ...
    if ((a.profile.smoking_preference === 'reject' && b.profile.smoking_preference === 'accept') ||
        (b.profile.smoking_preference === 'reject' && a.profile.smoking_preference === 'accept')) {
        smokingPenalty = 0.3;
    }
    const raw = commSim * 0.18 +
        repairSim * 0.12 +
        goalMatch * 0.18 +
        click * 0.20 +
        sameAcademy * 0.05 +
...
function baselineScore(a, b, pa, pb) {
...
function feedbackFromGroundTruth(score, rng) {
    ...
    if (score >= 60)
        return pickWeighted(rng, [
            { value: 'liked', weight: 30 },
            { value: 'chatted', weight: 15 },
            { value: 'passed', weight: 35 },
            { value: 'dismissed', weight: 18 },
            { value: 'met', weight: 2 },
        ]);
    if (score >= 40)
        return pickWeighted(rng, [
            { value: 'liked', weight: 12 },
            { value: 'passed', weight: 45 },
            { value: 'dismissed', weight: 35 },
            { value: 'blocked', weight: 3 },
            { value: 'chatted', weight: 5 },
        ]);
    return pickWeighted(rng, [
        { value: 'passed', weight: 35 },
        { value: 'dismissed', weight: 50 },
        { value: 'blocked', weight: 15 },
    ]);
}
export function generateMatches(users, portraits, count, seed) {
    ...
    while (out.length < count && attempts < maxAttempts) {
        ...
        const reveal_at = new Date(new Date('2026-04-15T00:00:00Z').getTime() + randInt(rng, 0, 21 * 24 * 3600 * 1000)).toISOString();
        out.push({
